        ...
        public void OnPostBtRefrescar()
        {
            try
            {
                if (IclientesNegocio == null)
                    return;
                var usuario = HttpContext.Session.GetString("Usuario");
                Lista = IclientesNegocio.Consultar(usuario!);
                cliente = null;
            }
            catch (Exception ex)
            {
        ...
        public void OnPostBtGuardar()
        {
            try
            {
                ...
                if (cliente == null)
                    return;
                if (cliente.id == 0)
                    cliente = IclientesNegocio!.Guardar(cliente!,usuario!);
                else
                    cliente = IclientesNegocio!.Modificar(cliente!,usuario!);
                if (cliente.id == 0)
                    return;
                OnPostBtRefrescar();
            }
        ...
        public void OnPostBtBorrar(int data)
        {
            try
            {
                if(cliente==null) 
                    return;
                var usuario = HttpContext.Session.GetString("Usuario");
                cliente = IclientesNegocio!.Borrar(cliente!,usuario);
                OnPostBtRefrescar();
            }
            catch (Exception ex)
            {
